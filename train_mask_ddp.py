...
def train(model, train_loader, optimizer, criterion, clip, device, rank):
    ...
    for i, (anchor_samples, anchor_targets, anchor_masks_enc, anchor_masks_dec, anchor_locflags, positive_samples, positive_targets,
        ...
        anchor_samples, anchor_targets, anchor_masks_enc, anchor_masks_dec, anchor_locflags, positive_samples, positive_targets, positive_masks_enc, positive_masks_dec, positive_locflags, negative_samples, negative_targets, negative_masks_enc, negative_masks_dec, negative_locflags = anchor_samples.to(
            ...
            device), positive_masks_dec.to(device), positive_locflags.to(device), negative_samples.to(device), negative_targets.to(
        ...
        if args.add_mapping:
            dist_loss = torch.mean((hidden_repr.squeeze().mean(dim=0) - llm_embed_mean.unsqueeze(0).repeat(len(hidden_repr), 1)) ** 2) + torch.mean((hidden_repr.squeeze().var(dim=0) - llm_embed_var.unsqueeze(0).repeat(len(hidden_repr), 1)) ** 2)*args.dist_weight
        else:
        ...
        loss_reg = nn.L1Loss()(reg_pred[anchor_targets == 0], anchor_samples[anchor_locflags == 1]) * args.reg_weight
        loss = loss_cls + loss_reg
        # calculate triplet loss
        anchor_embedding = model(anchor_samples, anchor_masks_enc, anchor_masks_dec, anchor_locflags, return_embedding=True)
        ...
        if (i % 20 == 0 or i == len(train_loader) - 1) and rank == 0:
            ...
            print(f"box gt {anchor_samples[0][anchor_locflags[0] == 1].flatten()}")
            print(f"+" * 20)
...
def evaluate(model, val_loader, criterion, device):
    ...
    with torch.no_grad():
        for i, (anchor_samples, anchor_targets, anchor_masks_enc, anchor_masks_dec, anchor_locflags, positive_samples, positive_targets,
            ...
            positive_embedding = model(positive_samples, positive_masks_enc, positive_masks_dec, positive_locflags, return_embedding=True)
            negative_embedding = model(negative_samples, negative_masks_enc, negative_masks_dec, negative_locflags, return_embedding=True)
            distance_positive = torch.pairwise_distance(anchor_embedding, positive_embedding)
            distance_negative = torch.pairwise_distance(anchor_embedding, negative_embedding)
            triplet_loss = F.relu(distance_positive - distance_negative + 0.2).mean() * args.triplet_weight
...
def run():
    ...
    train_dataset = SegmentationDataset(path="/home/pirenjie/data/coco/annotations/instances_train2017.json", transformation=args.transform, size=args.train_size)
    if n_gpus > 1:
    ...
    val_dataset = SegmentationDataset(path="/home/pirenjie/data/coco/annotations/instances_val2017.json", transformation=args.transform, size=args.val_size)
    ...
    for step in range(args.epoch):
        ...
        train_loss, train_loss_cls, train_loss_reg, train_loss_triplet, train_loss_dist = train(model, train_loader, optimizer,
                                                                               criterion, clip, device, local_rank)
        scheduler.step()
        ...
        if local_rank == 0:
            ...
            training_losses = [train_losses, train_losses_cls, train_losses_reg, train_losses_triplet, train_losses_dist]
