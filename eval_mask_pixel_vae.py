...
def evaluate(model, refer, device):
    ...
    with torch.no_grad():
        for i, ref_id in enumerate(ref_ids):
            ...
            generated = model(mask_pixel, noise_scale=0.1)
            # generated_masks.append((generated[0] >0.5).to(torch.float32).squeeze().cpu().numpy())
            generated_masks.append(generated[0].to(torch.float32).sigmoid().squeeze().cpu().numpy())
...
def run():
    ...
    model.load_state_dict(torch.load(
        f"/home/pirenjie/transformer-master/saved/pixelmask_resnet_dist0.01_noise0.01_triplet1-model.pt"))
