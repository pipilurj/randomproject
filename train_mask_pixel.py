...
def train(model, train_loader, optimizer, criterion, clip, device, rank, n_gpus):
    ...
    total_samples = 0
    cur_iters = 0
    for i, (inputs_dec, mask_pixel, targets, masks_dec) in enumerate(train_loader):
        cur_iters += 1
        ...
        cls_loss = criterion(cls_pred_reshape[cls_target != -1], cls_target[cls_target != -1]) * args.cls_weight
        ...
        if (i % 20 == 0 or i == len(train_loader) - 1) and rank == 0:
            ...
            print(f"train epoch cls loss {total_cls_loss / total_samples}")
    ...
    if n_gpus > 1:
        ...
        dist.reduce(torch.tensor(total_cls_loss, device=device), dst=0)
        ...
        if dist.get_rank() == 0:
            ...
            return average_loss, average_cls_loss, average_dist_loss
        else:
            return None, None, None
    else:
        return total_loss / total_samples, total_cls_loss / total_samples, total_dist_loss / total_samples
