...
def get_padded_sample_target_mask(samples):
    max_len = max([len(s["quant_poly11"]) for s in samples])
    samples_extended = dict()
    for key in samples[0].keys():
        samples_extended[key] = []
    targets_all, masks_enc_all, masks_dec_all, loc_flags_all = [], [], [], []
    for sample in samples:
        sample_copy = copy.deepcopy(sample)
        for key, val in sample_copy.items():
            # if "quant" in key:
            #     sample_copy[key] = np.concatenate((val, np.array([[0., 0.]])), axis=0)
            # elif "delta" in key:
            if key == "polygon":
                sample_copy[key] = np.concatenate((val, np.array([[0., 0.]])), axis=0)
            elif "quant" in key:
                sample_copy[key] = np.concatenate((val, np.array([-1])), axis=0)
            else:
                sample_copy[key] = np.concatenate((val, np.array([0])), axis=0)
        target_loc = [0 for _ in range(len(sample["quant_poly11"]))]  #
        target_loc.append(1)  # add eos token
        target_loc.append(-1)  # the extra <eos> token
        loc_flags = [1 for _ in range(len(sample["quant_poly11"]))]  # 1 represents location token
        loc_flags.append(3)  # 3 represents <eos>
        mask_enc = [1 for _ in range(len(sample["quant_poly11"]) + 2)]  # one additional mask for cls, another for <eos>
        mask_dec = [1 for _ in range(len(sample["quant_poly11"]) + 1)]  # one additional mask for bos
        mask_dec.append(0)  # mask out <eos>
        while len(sample_copy["quant_poly11"]) < max_len + 1:
            for key, val in sample_copy.items():
                if key == "polygon":
                    sample_copy[key] = np.concatenate((val, np.array([[0., 0.]])), axis=0)
                elif "quant" in key:
            ...
            loc_flags.append(0)
        for key, val in sample_copy.items():
            samples_extended[key].append(val)
        targets_all.append(target_loc)
        masks_enc_all.append(mask_enc)
        masks_dec_all.append(mask_dec)
        loc_flags_all.append(loc_flags)
        # break
    for key, val in samples_extended.items():
        if "quant" in key:
            samples_extended[key] = torch.tensor(numpy.stack(val), dtype=torch.long)
        else:
            samples_extended[key] = torch.tensor(numpy.stack(val), dtype=torch.float32)
    targets_all, masks_enc_all, masks_dec_all, loc_flags_all = numpy.array(targets_all), numpy.array(
        masks_enc_all), numpy.array(
        masks_dec_all), numpy.array(loc_flags_all)
    return samples_extended, torch.tensor(targets_all, dtype=torch.float32), torch.tensor(
...
class SegmentationCodebookDataset(Dataset):
    # class label for tokens:
    # loc: 0; sep: 1; eos: 2
    def __init__(self, path="/home/pirenjie/data/coco/annotations/instances_train2017.json", downsample_num_upper=40,
                 downsample_num_lower=40, transformation=True, size=None, num_bins=64):
        self.coco = COCO(path)
        annotations = self.coco.dataset["annotations"]
        self.legal_indices = list(range(len(annotations)))
        if size is not None:
            self.legal_indices = random.sample(self.legal_indices, size)
        self.legal_indices = [i for i in self.legal_indices if isinstance(annotations[i]["segmentation"], list) and all([len(anno) >= downsample_num_lower for anno in annotations[i]["segmentation"]])]
    ...
    def quantize_polygon(self, polygon):
        ...
        quant_poly = polygon * (self.num_bins - 1)
        quant_poly11, quant_poly21, quant_poly12, quant_poly22 = [], [], [], []
        delta_x1, delta_y1 = [], []
        for p in quant_poly:
            quant_poly11.append(self.loc_to_locid[f"<bin_{math.floor(p[0])}_{math.floor(p[1])}>"])
            quant_poly21.append(self.loc_to_locid[f"<bin_{math.ceil(p[0])}_{math.floor(p[1])}>"])
            quant_poly12.append(self.loc_to_locid[f"<bin_{math.floor(p[0])}_{math.ceil(p[1])}>"])
            quant_poly22.append(self.loc_to_locid[f"<bin_{math.ceil(p[0])}_{math.ceil(p[1])}>"])
            delta_x1.append(p[0] - math.floor(p[0]))
            delta_y1.append(p[1] - math.floor(p[1]))
        delta_x1 = np.array(delta_x1)
        delta_x2 = 1 - delta_x1
        delta_y1 = np.array(delta_y1)
        delta_y2 = 1 - delta_y1
        return {"polygon": polygon, "quant_poly11": np.array(quant_poly11, dtype="i"), "quant_poly21": np.array(quant_poly21, dtype="i"), "quant_poly12":  np.array(quant_poly12, dtype="i"),
                "quant_poly22":  np.array(quant_poly22, dtype="i"), "delta_x1": delta_x1, "delta_x2": delta_x2, "delta_y1": delta_y1,
                "delta_y2": delta_y2}
    ...
    def __getitem__(self, index):
        ...
        anchor_pack = self.quantize_polygon(
            segment_rescaled_anchor)
        pos_pack = self.quantize_polygon(
            segment_rescaled_positive)
        neg_pack = self.quantize_polygon(
            segment_rescaled_negative)

        return anchor_pack, pos_pack, neg_pack


if __name__ == "__main__":
    import time

    seg_dataset = SegmentationCodebookDataset("/home/pirenjie/data/coco/annotations/instances_train2017.json",
                                              transformation=False)
    loader = DataLoader(seg_dataset, batch_size=1000, collate_fn=custom_collate_fn)
    time_start = time.time()
    for anchor_samples, anchor_targets, anchor_masks_enc, anchor_masks_dec, anchor_locflags, positive_samples, positive_targets, positive_masks_enc, positive_masks_dec, positive_locflags, negative_samples, negative_targets, negative_masks_enc, negative_masks_dec, negative_locflags in loader:
        time_end = time.time()
