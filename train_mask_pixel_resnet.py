...
from models.model.resnet import ResNet50
import wandb
...
def pairwise_distance(x1, x2, p=2):
...
def train(model, train_loader, optimizer, criterion, clip, device, rank, n_gpus):
    llm_embed_mean, llm_embed_var = llm_embed.mean().unsqueeze(dim=0).to(device), llm_embed.var().unsqueeze(dim=0).to(
        device)
    model.train()
    total_loss, total_rec_loss, total_dist_loss, total_triplet_loss = 0.0, 0.0, 0.0, 0.0
    total_samples = 0
    cur_iters = 0
    torch.autograd.set_detect_anomaly(True)
    # for i, (mask_pixel, mask_pixel_pos, mask_pixel_neg) in enumerate(train_loader):
    for i, (mask_pixel, mask_pixel_pos, mask_pixel_neg) in enumerate(train_loader):
        cur_iters += 1
        # mask_pixel, mask_pixel_pos, mask_pixel_neg = mask_pixel.to(device), mask_pixel_pos.to(device), mask_pixel_neg.to(device)
        mask_pixel, mask_pixel_pos, mask_pixel_neg = mask_pixel.to(device), mask_pixel_pos.to(device), mask_pixel_neg.to(device)
        # calculate cls and reg losses
        reconstruction, hidden_repr = model(mask_pixel, return_embedding=True, noise_scale=args.noise_scale)
        ...
        if args.dist_weight > 0:
            mean, var = hidden_repr.mean(dim=1), hidden_repr.var(dim=1)
            dist_loss = kl_divergence(mean, var, llm_embed_mean, llm_embed_var) * args.dist_weight
        else:
            dist_loss = torch.tensor(0.).to(device)
        if args.triplet_weight != 0:
            _, hidden_repr_anchor = model(mask_pixel, return_embedding=True)
            _, hidden_repr_pos = model(mask_pixel_pos, return_embedding=True)
            _, hidden_repr_neg = model(mask_pixel_neg, return_embedding=True)

            distance_positive = pairwise_distance(hidden_repr_anchor, hidden_repr_pos)
            distance_negative = pairwise_distance(hidden_repr_anchor, hidden_repr_neg)
        ...
        loss = loss_reconstruction + dist_loss + triplet_loss
        optimizer.zero_grad()
        loss.backward()
        ...
        if (i % 20 == 0 or i == len(train_loader) - 1) and rank == 0:
            ...
            print(f"train epoch rec loss {loss_reconstruction.item()}")
            print(f"train epoch dist loss {dist_loss.item()}")
            print(f"train epoch triplet loss {triplet_loss.item()}")
            print(f"+" * 20)
    if n_gpus > 1:
        dist.reduce(torch.tensor(total_loss, device=device), dst=0)
        dist.reduce(torch.tensor(total_dist_loss, device=device), dst=0)
        dist.reduce(torch.tensor(total_triplet_loss, device=device), dst=0)
        ...
        if dist.get_rank() == 0:
            ...
            average_triplet_loss = total_triplet_loss / total_samples
            return average_loss, average_rec_loss, average_dist_loss, average_triplet_loss
        else:
            return None, None, None, None
    else:
        return total_loss / total_samples, total_rec_loss / total_samples, total_dist_loss / total_samples, total_triplet_loss / total_samples
...
def evaluate(model, val_loader, criterion, device, n_gpus):
    ...
    total_samples = 0
    with torch.no_grad():
        for i, (mask_pixel, mask_pixel_pos, mask_pixel_neg) in enumerate(val_loader):
            mask_pixel = mask_pixel.to(device)
            # calculate cls and reg losses
            reconstruction = model(mask_pixel)
            if args.recon_loss == "cross_entropy":
                reconstruction = reconstruction.view(-1)
                target = mask_pixel.view(-1)
...
def run():
    ...
    train_losses, train_losses_cls, train_losses_dist, train_losses_triplet, val_losses = [], [], [], [], []
    scheduler = CosineAnnealingLR(optimizer, T_max=args.epoch)
    for step in range(args.epoch):
        start_time = time.time()
        train_loss, train_cls_loss, train_dist_loss, train_triplet_losss = train(model, train_loader, optimizer, criterion, clip, device,
                                                            local_rank, n_gpus)
        scheduler.step()
        train_losses.append(train_loss)
        train_losses_cls.append(train_cls_loss)
        train_losses_dist.append(train_dist_loss)
        train_losses_triplet.append(train_triplet_losss)
        # if local_rank == 0:
        val_loss = evaluate(model, val_loader, criterion, device, n_gpus)
        val_losses.append(val_loss)
        if local_rank == 0:
            if not args.debug and not args.debug:
                wandb.log({"train_loss": train_loss,
                           "train_cls_loss": train_cls_loss,
                           "train_dist_loss": train_dist_loss,
                           "train_dist_loss": train_triplet_losss,
                           "val_loss": val_loss
                           }, step=step)
            if not args.no_save:
                if n_gpus > 1:
