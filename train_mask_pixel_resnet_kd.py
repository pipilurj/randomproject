...
def train(model, model_teacher, train_loader, optimizer, criterion, clip, device, rank, n_gpus):
    ...
    for i, (mask_pixel, mask_pixel_pos, mask_pixel_neg) in enumerate(train_loader):
        ...
        hidden_repr = model.module.encode(mask_pixel)
        with torch.no_grad():
            hidden_repr_teacher = model_teacher.encode(mask_pixel)
            reconstruction = model_teacher.decode(hidden_repr, img_size=(256,256))
        with torch.no_grad():
        ...
        l2_loss = F.mse_loss(hidden_repr, hidden_repr_teacher)
        loss = l2_loss
        optimizer.zero_grad()
        loss.backward()
        ...
        if (i % 20 == 0 or i == len(train_loader) - 1) and rank == 0:
            ...
            print(f"train epoch kl loss {l2_loss.item()}")
            print(f"+" * 20)
    if n_gpus > 1:
        dist.reduce(torch.tensor(total_loss, device=device), dst=0)
        dist.reduce(torch.tensor(total_kl_loss, device=device), dst=0)
        ...
        if dist.get_rank() == 0:
            ...
            return average_loss, average_rec_loss, average_kl_loss
        else:
            return None, None, None
    else:
        return total_loss / total_samples, total_rec_loss / total_samples, total_kl_loss / total_samples
...
def run():
    ...
    model_teacher.load_state_dict(torch.load("/home/pirenjie/transformer-master/saved/pixelmask_resnet_dist0.01_noise0.01_triplet1-model.pt"))
    ...
    train_losses, train_losses_recon, train_losses_kd, val_losses = [], [], [], []
    scheduler = CosineAnnealingLR(optimizer, T_max=args.epoch)
    for step in range(args.epoch):
        start_time = time.time()
        train_loss, train_recon_loss, train_kd_loss = train(model, model_teacher, train_loader, optimizer, criterion, clip, device,
                                                            local_rank, n_gpus)
        scheduler.step()
        train_losses.append(train_loss)
        train_losses_recon.append(train_recon_loss)
        train_losses_kd.append(train_kd_loss)
        # if local_rank == 0:
