...
def transform(polygons, w, h, margin=0.5):
    translation_range = [-25, 25]
    shearing_range = [-0.5, 0.5]
    transformed_polygons = []
    shearx, sheary = random.uniform(*shearing_range), random.uniform(*shearing_range)
    for polygon in polygons:
        ...
        transx, transy = random.uniform(-(max_x-margin_x), (w-min_x-margin_x)), random.uniform(-(max_y-margin_y), (h-min_y-margin_y))
...
class SegmentationMaskDataset(Dataset):
    ...
    def load_segment_scale(self, index):
        ...
        segments_interpolated = interpolate_polygons(segments_processed)
        ds_num = random.randint(self.downsample_num_lower, self.downsample_num_upper)
        # debug
        segments = downsample_polygons(segments_interpolated, ds_num=ds_num)
        segments_scaled = scale_segments(segments, np.array([w, h]))

        return [np.clip(segment, 0, 10000) for segment in segments_scaled], mask_pixel
    ...
    def __getitem__(self, index):
        segments, mask_pixel = self.load_segment_scale(index)
        segment_discrete = self.quantize_polygon(
            segments)

        return segment_discrete, mask_pixel
...
if __name__ == "__main__":
    ...
    loader = DataLoader(seg_dataset, batch_size=1, collate_fn=custom_collate_fn)
    ...
    segments, masks = [], []
    start = 10
    for i, (seg, mask, targets, _) in enumerate(loader):
        if len(masks) >= 9:
            break
        if i >= start:
